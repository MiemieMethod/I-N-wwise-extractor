...
def unpackFastPatches():
    VERSION = 1406
    for i in range(0, 17):
        if os.path.exists(f"output/FastPatchPaks/PatchPak_{i}_{VERSION}_P.pak"):
            if not os.path.exists(f"output/unpacked{i}"):
                os.mkdir(f"output/unpacked{i}")

            subprocess.run(['./quickbms_4gb_files', '-C', '-o', '-F', f'PatchPak_{i}_{VERSION}_P.pak',
                            r'./output/bms/unreal_tournament_4_0.4.27e_infinity_nikki.bms', f'./output/FastPatchPaks',
                            f'./output/unpacked{i}'],
                           encoding='utf-8')
        if os.path.exists(f"output/FastPatchPaks/PatchPakWithShader_{i}_{VERSION}_P.pak"):
            if not os.path.exists(f"output/unpacked{i}"):
                os.mkdir(f"output/unpacked{i}")

            subprocess.run(['./quickbms_4gb_files', '-C', '-o', '-F', f'PatchPakWithShader_{i}_{VERSION}_P.pak',
                            r'./output/bms/unreal_tournament_4_0.4.27e_infinity_nikki.bms', f'./output/FastPatchPaks',
                            f'./output/unpacked{i}'],
                           encoding='utf-8')
        if os.path.exists(f"output/FastPatchPaks/PatchPak_Default_Movie_{i}_{VERSION}_P.pak"):
            if not os.path.exists(f"output/unpacked{i}"):
                os.mkdir(f"output/unpacked{i}")

            subprocess.run(['./quickbms_4gb_files', '-C', '-o', '-F', f'PatchPak_Default_Movie_{i}_{VERSION}_P.pak',
                            r'./output/bms/unreal_tournament_4_0.4.27e_infinity_nikki.bms', f'./output/FastPatchPaks',
                            f'./output/unpacked{i}'],
                           encoding='utf-8')
