...
def decodeWems():
    ...
    for root, dirs, files in os.walk("output/unpacked45/X6Game/Content/Audio/Media"):
        for file in files:
            if file.endswith(".wem"):
                path = root.replace("\\", "/") + "/" + file
                short_path = path.replace("output/unpacked45/X6Game/Content/Audio/Media/", "").replace("wem", "wav")
...
def processBanks():
    ...
    with open("output/unpacked/X6Game/Content/Audio/SoundbanksInfo.json", "r", encoding="utf-8") as f:
        banks = json.load(f)
